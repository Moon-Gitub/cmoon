...
class SetupImporter extends AbstractImporter
{
    ...
    public function import(LegacyImportContext $ctx): void
    {
        ...
        if (! $ctx->dryRun) {
            $empresa->update([
                'razon_social' => ($legacy->razon_social ?? null) ?: $empresa->razon_social,
                'nombre_fantasia' => ($legacy->titular ?? null) ?: $empresa->nombre_fantasia,
                'cuit' => ($legacy->cuit ?? null) ?: $empresa->cuit,
                'condicion_iva' => CondicionIvaMapper::toCmoon($legacy->condicion_iva ?? null),
                'ingresos_brutos' => ($legacy->numero_iibb ?? null) ?: $empresa->ingresos_brutos,
                'inicio_actividades' => $this->parseDate($legacy->inicio_actividades ?? null) ?: $empresa->inicio_actividades,
                'domicilio' => ($legacy->domicilio ?? null) ?: $empresa->domicilio,
                'localidad' => ($legacy->localidad ?? null) ?: $empresa->localidad,
                'codigo_postal' => ($legacy->codigo_postal ?? null) ?: $empresa->codigo_postal,
                'telefono' => ($legacy->telefono ?? null) ?: $empresa->telefono,
                'email' => ($legacy->mail ?? null) ?: $empresa->email,
    ...
    public static function createEmpresaFromLegacy(object $legacy): Empresa
    {
        ...
        $attrs = [
            'razon_social' => ($legacy->razon_social ?? null) ?: 'Empresa importada',
            'nombre_fantasia' => $legacy->titular ?? null,
            'condicion_iva' => CondicionIvaMapper::toCmoon($legacy->condicion_iva ?? null),
            'ingresos_brutos' => $legacy->numero_iibb ?? null,
            'inicio_actividades' => (new self)->parseDate($legacy->inicio_actividades ?? null),
            'domicilio' => $legacy->domicilio ?? null,
            'localidad' => $legacy->localidad ?? null,
            'codigo_postal' => $legacy->codigo_postal ?? null,
            'telefono' => $legacy->telefono ?? null,
            'email' => $legacy->mail ?? null,
            'activa' => true,
        ];
