...
<div class="space-y-6">
    ...
    @else
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            ...
            {{-- Estadísticas --}}
            <div class="bg-slate-800 rounded-xl p-6">
                <h3 class="font-semibold mb-4">Sincronización</h3>
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm">Syncs exitosos hoy</span>
                        </div>
                        <span class="font-bold text-lg">{{ number_format($stats['syncs_hoy'] ?? 0) }}</span>
                    </div>
                    @if(($stats['errores_24h'] ?? 0) > 0)
                        <div class="flex items-center justify-between p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-sm text-red-400">Errores (24h)</span>
                            </div>
                            <span class="font-bold text-lg text-red-400">{{ $stats['errores_24h'] }}</span>
                        </div>
                    @endif
                </div>
            ...
            {{-- Ventas Tiendanube --}}
            <div class="bg-slate-800 rounded-xl p-6">
                <h3 class="font-semibold mb-4">Ventas Tiendanube</h3>
                <div class="space-y-4">
                    <div class="text-center p-4 bg-gradient-to-br from-purple-500/20 to-indigo-500/20 rounded-lg">
                        <div class="text-3xl font-bold text-white">${{ number_format($stats['total_ventas_tn'] ?? 0, 0, ',', '.') }}</div>
                        <div class="text-sm text-slate-400 mt-1">Total acumulado</div>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div class="p-3 bg-slate-700/50 rounded-lg text-center">
                            <div class="text-2xl font-bold">{{ $stats['ventas_importadas_hoy'] ?? 0 }}</div>
                            <div class="text-xs text-slate-400">Hoy</div>
                        </div>
                        <div class="p-3 bg-slate-700/50 rounded-lg text-center">
                            <div class="text-2xl font-bold">{{ $stats['ventas_importadas_mes'] ?? 0 }}</div>
                            <div class="text-xs text-slate-400">Este mes</div>
                        </div>
                    </div>
                </div>

                <div class="mt-4 pt-4 border-t border-slate-700">
                    <a href="{{ route('ventas.index', ['origen' => 'tiendanube']) }}"
                       class="text-sm text-indigo-400 hover:underline flex items-center gap-1">
                        Ver todas las ventas de Tiendanube
